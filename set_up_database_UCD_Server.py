...
def insert_data():
    ...
    # Link to DB
    engine = connect_db('localhost', '3306', 'DBus', 'root', 'team1010')
    ...
    df['stop_id_orig'] = df['stop_id_orig'].apply(lambda x: str(x).zfill(4))
    df['stop_id_dest'] = df['stop_id_dest'].apply(lambda x: str(x).zfill(4))
